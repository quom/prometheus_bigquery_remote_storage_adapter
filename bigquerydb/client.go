...
func (c *BigqueryClient) buildCommand(q *prompb.Query) (string, error) {
	...
	for _, m := range q.Matchers {
		...
		switch m.Type {
		case prompb.LabelMatcher_EQ:
			matchers = append(matchers, fmt.Sprintf(`IFNULL(JSON_VALUE(tags.%s), '') = '%s'`, m.Name, m.Value))
		case prompb.LabelMatcher_NEQ:
			matchers = append(matchers, fmt.Sprintf(`IFNULL(JSON_VALUE(tags.%s), '') = '%s'`, m.Name, m.Value))
		case prompb.LabelMatcher_RE:
			matchers = append(matchers, fmt.Sprintf(`REGEXP_CONTAINS(IFNULL(JSON_VALUE(tags.%s), ''), r'%s')`, m.Name, m.Value))
		case prompb.LabelMatcher_NRE:
			matchers = append(matchers, fmt.Sprintf(`not REGEXP_CONTAINS(IFNULL(JSON_VALUE(tags.%s), ''), r'%s')`, m.Name, m.Value))
		default:
			return "", errors.Errorf("unknown match type %v", m.Type)
		}
	}
